...
public class Server {
    ...
    private static ArrayList<Float> tiempos = new ArrayList<>();
    private static ArrayList<ServerHilo> users = new ArrayList<>();
    ...
    public static void main(String args[]) throws IOException {
        ...
        barrera = new CyclicBarrier(numeroDeClientes);
        try {
            ss = new ServerSocket(PUERTO); // Crear socket servidor
            System.out.println("\t[OK]");
            int sesId = 0;
            while (sesId <= numeroDeClientes) {
                Socket socket;
                socket = ss.accept();
                System.out.println("Nueva conexion entrante: "+socket);
                ServerHilo usuario = new ServerHilo(socket, sesId, users, tiempos);
                usuario.start();
                users.add(usuario);
                sesId++;
            }
    ...
    public static void enviarACliente(Message mensaje, ServerHilo cliente) throws IOException {
        cliente.sendM(mensaje);
    }
} 
